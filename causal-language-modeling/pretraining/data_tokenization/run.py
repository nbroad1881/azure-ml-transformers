...
def main():
    ...
    all_files = list(map(str, Path(args.text_files_dir).rglob(args.glob_pattern)))
    ...
    for start_idx in range(0, len(ds), args.num_samples_per_file):
        end_idx = min(start_idx + args.num_samples_per_file, len(ds))
